...
pub fn init_metrics(project_ref: Option<&str>) -> Result<(), BuildError> {
    let addr = std::env::var("METRICS_ADDR").unwrap_or_else(|_| "[::]:9000".to_string());
    let socket_addr: std::net::SocketAddr = addr.parse().expect("Invalid METRICS_ADDR");

    let mut builder = PrometheusBuilder::new().with_http_listener(socket_addr);
